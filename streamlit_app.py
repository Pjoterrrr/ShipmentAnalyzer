...
st.markdown(
    ...
    <div class="section-banner">
        <div class="section-kicker">Analityka zmian release'u</div>
...
def first_non_empty(series):
    ...
    return values.iloc[0] if not values.empty else "brak danych"
...
def build_context_items(
    ...
    return [
        ...
        {
            "label": "Zakres produktów",
            "value": f"{selected_products_count}/{total_products}",
            "copy": "aktywnych produktów",
        },
    ]
...
def render_empty_filtered_state():
    st.markdown(
        ...
        <div class="empty-state">
            <div class="empty-state-title">Brak danych po zastosowaniu filtrów</div>
            <div class="empty-state-copy">
                Poszerz zakres dat, przywróć produkty albo wyłącz filtr alertów, aby wrócić do pełnego obrazu zmian.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_dashboard_hero(prev_meta, curr_meta, date_basis, selected_start_date, selected_end_date, metrics):
    ...
    with hero_left:
        ...
        st.markdown(
            ...
            <div class="hero-card">
                {hero_logo_html}
                <div class="hero-kicker">Panel analityczny release'u</div>
                <div class="hero-title">Dashboard zmian dla PO {html.escape(str(curr_meta['po_number']))}</div>
                <p class="hero-copy">
                    Porównanie poprzedniego i aktualnego release'u w profesjonalnym układzie dashboardowym:
                    od kontekstu biznesowego, przez KPI i kluczowe wnioski, aż po analizę produktu,
                    macierz release'u oraz dane szczegółowe.
                </p>
                <div class="hero-stat-grid">
                    <div class="hero-stat">
                        <div class="hero-stat-label">Plik poprzedni</div>
                        <div class="hero-stat-value">{html.escape(str(prev_meta['file_name']))}</div>
                    </div>
                    <div class="hero-stat">
                        <div class="hero-stat-label">Plik aktualny</div>
                        <div class="hero-stat-value">{html.escape(str(curr_meta['file_name']))}</div>
                    </div>
                    <div class="hero-stat">
                        <div class="hero-stat-label">Wersja poprzednia</div>
                        <div class="hero-stat-value">v{html.escape(str(prev_meta['release_version']))}</div>
                    </div>
                    <div class="hero-stat">
                        <div class="hero-stat-label">Wersja aktualna</div>
    ...
    with hero_right:
        st.markdown(
            ...
            <div class="hero-card">
                ...
                <p class="hero-copy">
                    Oś dat: <strong>{html.escape(get_date_label(date_basis))}</strong><br>
                ...
                <div class="hero-stat-grid">
                    <div class="hero-stat">
                        <div class="hero-stat-label">Wiersze w zakresie</div>
                        <div class="hero-stat-value">{metrics['rows']:,}</div>
                    </div>
                    <div class="hero-stat">
                        <div class="hero-stat-label">Produkty w zakresie</div>
                        <div class="hero-stat-value">{metrics['products_in_scope']:,}</div>
                    </div>
                    <div class="hero-stat">
                        <div class="hero-stat-label">Alerty</div>
                        <div class="hero-stat-value">{metrics['alert_count']:,}</div>
                    </div>
                    <div class="hero-stat">
                        <div class="hero-stat-label">Bilans zmian</div>
        ...
        normalized.append(
            {
                ...
                "role": str(raw_user.get("role", "Użytkownik")).strip() or "Użytkownik",
...
def load_auth_config():
    ...
    try:
        ...
        if secret_users:
            return secret_users, "sekrety Streamlit"
    except Exception:
        pass
...
def _legacy_render_sidebar_user():
    ...
    st.sidebar.markdown(
        ...
        <div class="sidebar-user-card">
            ...
            <div class="sidebar-user-name">{auth_user.get('display_name', 'Użytkownik')}</div>
            <div class="sidebar-user-role">{auth_user.get('role', 'Użytkownik')} · {auth_user.get('username', '')}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with left_col:
        ...
        st.markdown(
            ...
            <div class="login-brand-card">
                {logo_html}
                <div class="login-kicker">Bezpieczny dostęp</div>
                ...
                <div class="login-points">
                    <div class="login-point">
                        <div class="login-point-title">Nowoczesny panel analityczny</div>
                        <div class="login-point-copy">Czytelne KPI, alerty, wykresy i macierz zmian z wyraźną hierarchią informacji.</div>
                    </div>
                    <div class="login-point">
                        <div class="login-point-title">Raport gotowy do pobrania</div>
                        <div class="login-point-copy">Filtrowane dane CSV i raport Excel przygotowany do dalszej pracy operacyjnej.</div>
                    </div>
                    <div class="login-point">
                        <div class="login-point-title">Prosty dostęp dla zespołu</div>
                        <div class="login-point-copy">Ta sama aplikacja może działać lokalnie, w sieci firmowej lub jako wdrożenie online.</div>
                    </div>
                </div>
            </div>
            """,
        ...
        st.markdown(
            ...
            <div class="login-form-card">
                ...
                <div class="login-form-copy">
                    Użyj swojego loginu i hasła, aby otworzyć panel analityczny. Dane dostępowe są
                    przechowywane lokalnie w konfiguracji aplikacji.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
...
def render_sidebar_user():
    ...
    st.sidebar.markdown(
        ...
        <div class="sidebar-user-card">
            <div class="sidebar-user-label">Aktywna sesja</div>
            <div class="sidebar-user-name">{html.escape(str(auth_user.get('display_name', 'Użytkownik')))}</div>
            <div class="sidebar-user-role">{html.escape(str(auth_user.get('role', 'Użytkownik')))} &middot; {html.escape(str(auth_user.get('username', '')))}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def load_release(file_bytes, file_name):
    ...
    if missing_columns:
        raise ValueError(
            "W arkuszu Raw brakuje wymaganych kolumn: " + ", ".join(missing_columns)
        )
...
def write_summary_sheet(
    ...
    worksheet["D4"] = "Poprzednia ilość"
    worksheet["E4"] = total_prev
    worksheet["D5"] = "Aktualna ilość"
    worksheet["E5"] = total_curr
    worksheet["D6"] = "Bilans zmian"
    worksheet["E6"] = total_delta
    worksheet["D7"] = "Liczba alertów"
    worksheet["E7"] = alert_count
    worksheet["D8"] = "Zmienne produkty"
    worksheet["E8"] = products_changed
    worksheet["D9"] = "Nowe pozycje"
    worksheet["E9"] = new_positions
    worksheet["D10"] = "Usunięte pozycje"
    worksheet["E10"] = removed_positions

    worksheet["A13"] = "Kluczowe wnioski"
    ...
    worksheet["A20"] = "Najważniejsze zmiany produktów"
    worksheet["A20"].font = Font(size=13, bold=True, color="0F172A")
    worksheet["A21"] = "Numer części"
    worksheet["B21"] = "Opis produktu"
    worksheet["C21"] = "Poprzednia ilość"
    worksheet["D21"] = "Aktualna ilość"
    worksheet["E21"] = "Bilans zmian"
    worksheet["F21"] = "Liczba alertów"
...
def to_excel_bytes(
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        pd.DataFrame().to_excel(writer, sheet_name="Podsumowanie", index=False)
        detail_export.to_excel(writer, sheet_name="Dane szczegółowe", index=False)
        current_matrix_export.to_excel(writer, sheet_name="Macierz aktualna", index=False)
        delta_matrix_export.to_excel(writer, sheet_name="Mapa delta", index=False)

        summary_sheet = writer.book["Podsumowanie"]
        write_summary_sheet(
            summary_sheet,
            prev_meta,
            curr_meta,
        ...
        delta_heatmap_sheet = writer.book["Mapa delta"]
...
def render_loaded_dashboard(prev_meta, curr_meta, result):
    ...
    render_sidebar_section(
        "Panel analityczny",
        "Panel filtrowania",
        "Wybierz oś dat, zawęź listę produktów, ustaw kierunek zmiany i zakres okresu. Logika porównawcza oraz eksportowa pozostaje bez zmian.",
    )
    if logo_available():
    ...
    selected_products = st.sidebar.multiselect(
        ...
        placeholder="Wybierz jeden lub wiele produktów",
    )
    ...
    if available_dates.empty:
        st.error("Brak poprawnych dat do analizy dla wybranej osi. Sprawdź dane w arkuszu Raw.")
        return
    ...
    selected_date_input = st.sidebar.date_input(
        ...
        min_value=min_date,
        max_value=max_date,
        help="Wybierz jeden dzień albo pełny zakres dat do analizy.",
    )
    ...
    render_sidebar_section(
        ...
        (
            f"{metrics['rows']:,} wierszy, {metrics['products_in_scope']:,} produktów i "
            f"{metrics['alert_count']:,} alertów w okresie {format_period_label(selected_start_date, selected_end_date)}."
        ),
    )
    ...
    if filtered_df.empty:
        render_section_header(
            ...
            "Kontekst i pliki są poprawnie wczytane, ale aktualny zestaw filtrów nie zwrócił rekordów do analizy.",
        )
    ...
    render_section_header(
        "Przegląd zarządczy",
        "Karty KPI",
        "Nowoczesny zestaw kart pokazujący punkt odniesienia, aktualny wolumen, skalę alertów oraz nowe lub usunięte pozycje.",
    )
    render_status_pills(
        metrics["total_delta"],
        metrics["alert_count"],
    ...
    with metric_cols[0]:
        render_kpi_card(
            ...
            "Bazowy release do porównania",
        )
    with metric_cols[1]:
        render_kpi_card(
            "Aktualna ilość",
            format_number(metrics["total_curr"]),
            f"Zmiana względem poprzedniego release'u {format_signed_int(metrics['total_delta'])}",
    ...
    with metric_cols[2]:
        render_kpi_card(
            ...
            "Różnica między aktualnym a poprzednim release'em",
            tone="positive" if metrics["total_delta"] > 0 else "negative" if metrics["total_delta"] < 0 else "neutral",
        )
    with metric_cols[3]:
        render_kpi_card(
            ...
            f"{metrics['alert_ratio']:.1%} rekordów przekracza próg {THRESHOLD}%",
            tone="alert",
        )
    with metric_cols[4]:
        render_kpi_card(
            ...
            f"z {metrics['products_in_scope']:,} aktywnych produktów",
        )
    with metric_cols[5]:
        render_kpi_card(
            "Nowe pozycje",
            f"{metrics['new_positions']:,}",
            f"w tym {metrics['new_products']:,} produktów",
    ...
    with metric_cols[6]:
        render_kpi_card(
            ...
            f"w tym {metrics['removed_products']:,} produktów",
    ...
    render_section_header(
        "Kluczowe wnioski",
        "Kluczowe wnioski",
        "Blok kart z wnioskami prowadzi od najważniejszych sygnałów biznesowych do dalszej analizy szczegółowej.",
    )
    if key_findings:
        ...
        st.info("Brak insightów do pokazania dla bieżącego zestawu filtrów.")
    ...
    with dashboard_tab:
        render_section_header(
            "Dashboard",
            "Flow analityczny od ogółu do szczegółu",
            "Najpierw pełny trend poprzedniego i aktualnego release'u, niżej delta i struktura zmian, a następnie największe ruchy, alerty oraz tabele priorytetowe.",
        )
        st.altair_chart(
        ...
        with trend_left:
            ...
            st.caption("Bilans zmian pokazuje, w których dniach release najmocniej odchylił się od poprzedniej wersji.")
            st.altair_chart(
        ...
        with trend_right:
            ...
            st.caption("Szybki podgląd proporcji wzrostów, spadków i pozycji bez zmian w aktualnym oknie analizy.")
            st.altair_chart(
        ...
        render_section_header(
            "Alerty priorytetowe",
            "Produkty z największą liczbą alertów",
            "Tabela jest agregowana po produkcie i domyślnie sortowana malejąco po liczbie alertów, z dodatkowym priorytetem dla największych zmian wolumenu.",
        )
        if top_alert_products.empty:
            st.info("Brak produktów z alertami w bieżącym zakresie.")
        else:
        ...
        render_section_header(
            ...
            "Najmocniejsze ruchy po produkcie, uporządkowane według bezwzględnej zmiany wolumenu i gotowe do szybkiego przeglądu operacyjnego.",
        )
    ...
    with product_tab:
        render_section_header(
            "Analiza produktu",
            "Raport produktu",
            "Szczegółowa analiza pojedynczego numeru części z osobnym trendem, deltą i tabelą dla wszystkich dat w aktywnym zakresie.",
        )
        selected_product_label = st.selectbox(
            "Wybierz produkt",
            options=product_summary["Product Label"].tolist(),
            help="Lista odzwierciedla aktualny zakres filtrów z sidebaru.",
        )
        ...
        with product_metric_cols[3]:
            render_kpi_card(
                ...
                f"{product_metrics['alert_ratio']:.1%} rekordów produktu",
        ...
        render_section_header(
            ...
            "Tabela utrzymuje pełny kontekst dat, procent zmian, kierunku ruchu oraz flag alertów dla pojedynczego produktu.",
        )
    ...
    with matrix_tab:
        render_section_header(
            "Macierz release'u",
            "Macierz release'u",
            "Widok tabelaryczny w stylu panelu planistycznego. Wybierz metrykę i porównaj rozkład wartości po datach dla aktywnego zestawu filtrów.",
        )
        ...
        st.caption(
            "Macierz zachowuje dotychczasową logikę liczenia i tylko poprawia oprawę sekcji oraz czytelność wyboru metryki."
        )
    ...
    with detail_tab:
        render_section_header(
            ...
            "Pełny podgląd filtrowanych rekordów z limitem podglądu oraz gotowymi eksportami CSV i Excel bez zmiany dotychczasowej logiki raportowej.",
        )
